...
public class EcmDealogicEntityConverter {
	...
	private static EcmDealogic createUpdateEcmDealogicObject(EcmDealogic e1, String columnStandardizedName,
		...
		if (columnStandardizedName.equals(EcmDealogicFieldConstant.ORDERDATETIME)) {
			e1.setOrderDateTime(setTimeStamp(formatedColumnValue));
		}
	...
	private static Timestamp setTimeStamp(String value) {
		if (StringUtils.hasLength(value)) {
			String dateFormat = "dd/MMM/yyyy hh:mm:ss.SSS";
			System.out.println("TIME STAMP :------------------------  "+value);
			try {
				return new Timestamp(
						new SimpleDateFormat(dateFormat).parse(String.valueOf(value)).getTime());
			} catch (Exception e) {
				return null;
			}
